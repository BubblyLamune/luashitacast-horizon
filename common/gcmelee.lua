...
local GardenBanglesJobs = T({
	"MNK",
	"THF",
	"DRK",
	"RNG"
});
local PresidentialHairpinJobs = T({
	"MNK",
	"THF",
	"BST",
	"NIN",
...
function gcmelee.DoDefault(max_hp_in_idle_with_regen_gear_equipped)
	...
	if player.MainJob == "PLD" or player.MainJob == "NIN" or player.MainJob == "DRK" then
		if player.SubJob == "NIN" or player.MainJob == "NIN" then
			...
			if GetShadowCount() == 0 then
				...
				if gcdisplay.IdleSet == "Alternate" then
					gFunc.EquipSet("IdleALTDT");
				end;
			end;
		end;
	end;
	if player.Status == "Idle" then
		if player.HPP < 50 then
			if MuscleBeltJobs:contains(player.MainJob) then
		...
		if player.HP < max_hp_in_idle_with_regen_gear_equipped then
			...
			if environment.Time >= 6 and environment.Time < 18 then
				if GardenBanglesJobs:contains(player.MainJob) then
					gFunc.EquipSet("garden_bangles");
				end;
			end;
			if conquest:GetOutsideControl() then
				if PresidentialHairpinJobs:contains(player.MainJob) then
	...
	if isDPS then
		if gcdisplay.IdleSet == "Normal" or gcdisplay.IdleSet == "Alternate" or gcdisplay.IdleSet == "LowAcc" or gcdisplay.IdleSet == "HighAcc" then
			if player.Status == "Engaged" then
				...
				local mjollnirHaste = gData.GetBuffCount(580) > 0;
				if aftermath then
...
function gcmelee.SetupInterimEquipSet(sets)
	...
	if gcdisplay.IdleSet == "FireRes" then
		gFunc.InterimEquipSet(sets.FireRes);
	end;
	if gcdisplay.IdleSet == "IceRes" then
		gFunc.InterimEquipSet(sets.IceRes);
	end;
	if gcdisplay.IdleSet == "LightningRes" then
		gFunc.InterimEquipSet(sets.LightningRes);
	end;
	if gcdisplay.IdleSet == "EarthRes" then
		gFunc.InterimEquipSet(sets.EarthRes);
	end;
end;
